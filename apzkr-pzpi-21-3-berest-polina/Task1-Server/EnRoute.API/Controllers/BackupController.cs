    ...
    public class BackupController : Controller
    {
        ...
        public IActionResult CreateDbBackup()
        {
            try
            {
                ...
                var backupFileName = $"enroute_db_backup_{DateTime.UtcNow:yyyyMMddHHmmss}.bak";

                var backupPath = Path.Combine(backupDirectoryPath, backupFileName);
                var backupCommand = $"BACKUP DATABASE enroute TO DISK = '{backupPath}'";
        ...
        public IActionResult GetDbBackups()
        {
            try
            {
                ...
                var backupFiles = backupDirectory.GetFiles("enroute_db_backup_*.bak")
                    .Select(fileInfo => new
                    {
